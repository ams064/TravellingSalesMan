...
typedef std::vector<std::vector<long>> Graph;
typedef std::vector<unsigned> Journey;
typedef std::vector<vector< pair<long,unsigned>>> Mem;

pair<long, vector<unsigned>>& tsp_recurse_mem(unsigned i, Graph& graph, Mem& mem, unsigned int mask, unsigned int limit);
